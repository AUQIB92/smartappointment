...
export async function POST(request) {
  try {
    ...
    console.log(`WhatsApp Verify API - Verifying OTP: ${otp} for mobile: ${mobile}`);
    
    // Verify OTP
    const verificationResult = await verifyOTP(mobile, 'whatsapp', otp);
    
    if (!verificationResult.success) {
      console.log('WhatsApp Verify API - Invalid OTP:', verificationResult.error);
      return NextResponse.json(
    ...
    if (isRegistration) {
      ...
      const updateResult = await updateUserAfterVerification(mobile, 'whatsapp', updateData);
      
      if (!updateResult.success) {
        console.error('WhatsApp Verify API - Failed to update user:', updateResult.error);
        return NextResponse.json(
